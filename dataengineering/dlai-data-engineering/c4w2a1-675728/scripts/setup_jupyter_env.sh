...
#     "
#     aws ec2 associate-iam-instance-profile --iam-instance-profile Name=Cloud9EC2InstanceProfile --instance-id $instance_id
# else
#     echo "    
#     replacing:" $inst_prof "by Cloud9EC2InstanceProfile    
#     "
#     aws ec2 replace-iam-instance-profile-association --iam-instance-profile Name=Cloud9EC2InstanceProfile --association-id $(aws ec2 describe-iam-instance-profile-associations --query 'IamInstanceProfileAssociations[?contains(InstanceId, `'$instance_id'`) == `true`].AssociationId' --output text)
...
export sg_modification_status=$(aws ec2 authorize-security-group-ingress --group-id $security_group_id --protocol tcp --port 5432 --cidr 0.0.0.0/0 --query 'Return' --output text)
